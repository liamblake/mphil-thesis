...
\begin{theorem}[Linearisation error is bounded]\label{thm:main}
	...
	Then, for any \(r \geq 1\) such that \(\delta_{2r} < \infty\) and \(t \in [0,T]\), there exist constants
	\[
		D_1\!\left(r,t, K_{\nabla u}, K_{\sigma}\right), \, D_2\!\left(r,t, K_{\nabla u}\right), \, D_3\!\left(r,t, K_{\nabla u}\right) \in [0, \infty)
	\]
	independent of \(x\) and \(x_0\) such that for all \(\epsilon > 0\),
	\begin{equation}
...
\begin{corollary}[Distribution of the linearisation solution]\label{cor:limit_moments}
	The It\^o integral term in the linearised solution \cref{eqn:linear_sol} follows a Gaussian distribution independently of \(x\), namely
	\[
		\int_0^t{L\!\left(x_0,\tau\right)\dif W_\tau} \isGauss{0, \int_0^t{L\!\left(x_0, \tau\right)L\!\left(x_0, \tau\right)^{\T}\dif\tau}}.
	\]
	The mean of the linearisation solution is
	\begin{equation}
		\avg{l_t^{(\epsilon)}} = F_0^t\!\left(x_0\right) + \nabla F_0^t\!\left(x_0\right) \avg{x - x_0}.
		\label{eqn:mean_expl_eqn}
	\end{equation}
	The \(n\times n\) covariance matrix of the linearisation solution is given explicitly by
	\begin{equation}
...
\begin{definition}[Stochastic sensitivity in \(\R^n\)]\label{def:ss_Rn}
	The \emph{stochastic sensitivity} is the scalar field \(S^2\colon \R^n \times [0,T] \to \left[0, \infty\right)\) given by
	\begin{equation*}
...
The anisotropic uncertainty in 2-dimensions \citep{Balasuriya_2020_StochasticSensitivityComputable} is the direction-dependent projection (before optimising over all directions in \Cref{def:ss_Rn}).
...
We show here that our results in \(n\)-dimensions are a generalisation of the 2-dimensional ones by \citet{Balasuriya_2020_StochasticSensitivityComputable}, which moreover establishes Gaussianity as well as an explicit expression for the uncertainty measure.
...
It has already been shown that, in the fluid flow context, stochastic sensitivity can identify coherent regions in 2-dimensions \citep{BadzaEtAl_2023_HowSensitiveAre, Balasuriya_2020_StochasticSensitivityComputable}.
...
The spatial gradient (with respect to the initial condition) of the flow map solves the equation of variations associated with \cref{eqn:ode_det}, i.e.\ recall \cref{eqn:eqn_of_variations}:
\[
...
\begin{proof}
	...
	\]
	Each \(\mathcal{I}[g_{ij}]\) is an It\^o integral of a deterministic, scalar-valued function with respect to a 1-dimensional Brownian motion, which is well-known to be a Gaussian process (e.g.\ see Lemma 4.3. of \citet{Applebaum_2004_LevyProcessesStochastic}).

...
The transition rates are listed in \Cref{fig:sir_transition}, with the diagram showing how an individual in the population moves through the three stages.
...
As with stochastic differential equations, CTMC models cannot generally be solved (in the sense of finding distributions across the state space at certain times, determining the long-term behaviour, etc.) analytically, but can be simulated \citep{Gillespie_1977_ExactStochasticSimulation}.
We summarise a method for simulating from a CTMC in \Cref{app:ctmc_sim}, which is used in the two examples in this section.
...
A CTMC is density dependent when the transition rates only depend on \(\frac1M X_t\) and not \(X_t\) directly for a population process \(X_t\) evolving with a fixed population size \(M\)\lb{Reconsider this sentence}.
Similar results for relaxations of these conditions exist \citep{Pollett_1990_ModelInterferenceSearching}, however.
We briefly summarise these results here---\Cref{app:epi_kurtz} provides further details---and can draw analogies with our work.
...
For example, the SIR model is a density dependent process, with the fluid limit
\begin{equation}
...
The fluid limit \cref{eqn:ctmc_ode} is parallel to the convergence of a small-noise SDE solution to the corresponding deterministic ODE solution, i.e. \(y_t^{(\epsilon)} \to y_t^{(0)}\) as \(\epsilon \downarrow 0\) in the notation of \Cref{ch:linear_theory}.
...
We shall now demonstrate some of these calculations and postulate future applications of stochastic sensitivity and our mixture model algorithm.
In \Cref{fig:sir_gauss_rels}, we show histograms of \(10000\) realisations of the population-scaled SIR model for four different population sizes.
...
This should be compared with \Cref{fig:sine_hists,fig:1d_mult_hists,fig:y_hists}, where we verified heuristically that the solutions to small-noise SDEs approached the Gaussian solutions of their respective linearisation.
...
We discussed in \Cref{sec:disc_bc} the boundary complication in our SDE linearisation framework and suggested that appropriately accounting for boundary conditions and absorbing states would be a valuable avenue for future work.
...
We can compute the covariance matrix of the solution to any diffusion limit \cref{eqn:ctmc_sde}, which satisfies the covariance differential equation \cref{eqn:pi_ode} from \Cref{ch:linear_theory} with \(Q\) in place of \(u\) and \(g\) in place of \(\sigma\).
...
In \Cref{fig:sir_s2}, we plot the computed stochastic sensitivity for the SIR model and compare the values with the corresponding empirical value (the operator norm of the sample covariance matrix) from 1000 realisations of the population process, for varying population size \(M\).
...
Finally, we will show the computation of the Gaussian approximation arising from the diffusion limit on a 5-dimensional model.
...
This model is far more complicated than the SIR example; \Cref{fig:ebola_transition} provide the transition events and a diagram corresponding to the evolution of an individual through the stages of the infection.

...
\td{Add a sentence referring to how these come up in practice, e.g. by citing \citet{SuraEtAl_2005_MultiplicativeNoiseNonGaussianity,del-Castillo-Negrete_1998_AsymmetricTransportNonGaussian,BraccoEtAl_2000_VelocityProbabilityDensity}.}
...
\[
	\dod{\var{l_t}}{t} =
\]
We plot the density of the resulting Gaussian approximation in dashed red in \Cref{fig:bene_gauss}.
...
\td{Weighting - allows for much more fun}
...
A key part of the algorithm is deciding \emph{when} to split, a criterion which should decide when the Gaussian approximation is no longer a reasonable representation of the nearby solution behaviour.
We do not provide a specific choice for this splitting criterion, and instead save a brief discussion of some options for later in \Cref{sec:gmm_split_disc}.
...
\begin{figure}
	...
	\begin{tikzpicture}[node distance=70pt]
		...
		\node (e) [rectangle, right of=b, below of = d, draw=black, align=center, yshift = -80pt] {\textbf{Split} \\ Use the splitting method to \\ construct \(x^{(K + 1)}, \dotsc, x^{(K+S)}\) \\ and the covariances \(\Sigma^{(K + 1)}, \dotsc, \Sigma^{(K + S)}\) \\ such that \cref{eqn:cov_split_points} is satisfied. \\
			Set \(w^{(K + 1)} = \dotsb = w^{(K + s)} = w^{(i)} / (S + 1)\) \\ and \(w^{(i)} = w^{(i)} / (S + 1)\) \\
			Set \(t^{(K + 1)} = \dotsb = t^{(K + S)} = t^{(i)}\) \\
			Set \(K = K + S\)};

		\node (f) [rectangle, rounded corners, left of=b, draw=black, align = center, xshift = -70pt] {\textbf{Stop}};

		\draw [arrow] (s) -- (a);
		\draw [arrow] (a) -- (b);

		\draw [arrow] (b) -- node[anchor=east] {yes} (c);
		\draw [arrow] (b) -- node[anchor=south] {no} (f);

		\draw [arrow] (c) -- (d);
		\draw [arrow] (d) |- node[anchor=east, yshift=-50pt] {yes} (b);

		\draw [arrow] (d) -- node[anchor=east] {no} (e);
		\draw (e) |- (b);

		% Outline!
		\node[inner sep=10pt, draw=red,fit = (b) (c) (d) (e), rounded corners] (fit) {};

...
\begin{enumerate}
	...
	\item While \(\tau^{(i)} < t\) for any \(i = 1,\dotsc, N\), iterate the following;
	      ...
	      \begin{enumerate}
		      ...
		      \item Otherwise, if \(\tau^{(i)} < t\), construct \(K\) points \(\hat{x}^{(1)},\dotsc,\hat{x}^{(K)}\) with weights \(\hat{w}^{(1)}, \dotsc, \hat{w}^{(K)}\) that preserve the propagated mean and covariance (i.e.\ satisfying \cref{eqn:cov_split_points}).

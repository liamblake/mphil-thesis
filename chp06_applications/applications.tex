...
We now apply the theoretical developments of \Cref{ch:linear_theory} and the mixture model algorithm of \Cref{ch:gmm} to two different applications, using models constructed from observed data.
We do not investigate either application in detail; rather, the aim is to demonstrate the applicability of the theory to a range of examples.
...
This model is 2-dimensional, and in the spirit of stochastic sensitivity we construct a deterministic model and use the solving trajectories to approximate the solution to an ``improved'' stochastic counterpart that accounts for measurement error and unresolved effects.

The second example is a compartmental model for the spread of Ebola.
\td{Overview of epi model}


\section{The Hellinger distance}
Before moving into the examples, however, we must first provide a measure to quantitatively evaluate the performance of the Gaussian approximation and our mixture model.
...
The sea surface height is proportional to the streamfunction for the surface flow, provided that the surface flow is treated as two-dimensional, and the constant of proportionality varies with latitude \citep{Park_2004_DeterminationSurfaceGeostrophic,DoglioniEtAl_2021_SeaSurfaceHeight}.
...
We convert the measured velocities to degrees per day via the following transformation: at longitude \(\lambda\) and latitude \(\phi\) (both in radians), the components \(u_{\mathrm{m}}\) and \(v_{\mathrm{m}}\) in metres per second are transformed as \citep{Capderou_2014_HandbookSatelliteOrbits}
...
\td{Describe the behaviour once of thep lot is there}
...
\td{Rewrite the following couple of sentences}
The error varies with time, but retains the main structure in that it appears highest along the Gulf Stream, which is not a surprise as this region contains the most complicated part of the flow.
...
Our choice for the diffusion matrix \(\sigma\) comes from the intuition that \(\sigma\sigma^{\T}\) represents the instantaneous variance of the noise in our stochastic differential equation.
In \cref{eqn:natl_sde} we have a stochastic model for the evolution of trajectories that is intended to improve over \cref{eqn:natl_ode} by accounting for measurement error and unresolved sugrid effects.
...
\subsection{Exploring a single trajectory}\label{sec:na_single}
...
To evaluate the Gaussian approximation over the time-evolution of the trajectory and the stochastic solution,
\Cref{fig:natl_hell} plots the estimated Hellinger distance between the Gaussian approximation and Euler-Maruyama samples solving \eqref{eqn:natl_sde}.
...
We now compute the stochastic sensitivity field.
...
\begin{figure}
	...
	\caption{The stochastic sensitivity fields (left) computed from the linearisation covariance matrix on a grid of initial conditions, at two different resolutions.
		...
		From each field, robust sets are extracted with a threshold of \(R = 2^\circ\) and shown on in cyan the right.
...
Notably, parts of the stream are \emph{not} included in these sets, due to the larger uncertainty.
...
To conclude this example, we will explore a simple implementation of the mixture model algorithm described in \Cref{ch:gmm} to predict the distribution of stochastic solutions about the single trajectory in \Cref{sec:na_single}.
...
\td{Need to find the best place for this paragraph}
...
\begin{figure}
	...
	\caption{The best-fitting mixture model with a single split at \(t_s = 33/24\) (9am \DTMdisplaydate{2021}{01}{02}{-1}).
		The joint histogram (centre) includes the component means (as red points) and contours of the mixture density.
...
\Cref{fig:na_1split_hell} plots, against the split time \(t_s\), the Hellinger distance between each mixture model and a set of 10000 EM samples.
With a single split, each mixture model consists of 5 equally weighted components: the single Gaussian component evolved from the initial condition, and the 4 additional components resulting from the splitting step.
...
Comparing directly to the single Gaussian in \Cref{fig:natl_em_3}, the latitudinal marginal is much improved with a smaller variance\lb{improve this!}.
...
\begin{figure}
	...
	\caption{The best-fitting mixture model (in red) with two splits on each component, over histograms of 10000 Euler-Maruyama samples.
		The central joint histogram shows contours of the mixture density, with the component means indicated by the red circles.
...
Five components may not be sufficient to fully capture the shape and spread of the empirical distribution, so we now consider adding an additional splitting step to the mixture model.
After the first split at time \(t_{s,1}\), we will split each new trajectory at a later time \(t_{s,2}\).
For simplicity, each of the five trajectories are split at the \emph{same} time \(t_{s,2}\), but in general the algorithm allows for each trajectory to be split at different times.
Each mean-covariance pair is split at time \(t_{s,2}\) into four additional points, again using the canonical sigma points \cref{eqn:uhlman_sigma}.
Once again, we consider a range of times for \(t_{s,1}\) and \(t_{s,2}\), construct the mixture model using each pair of times, and compute the Hellinger distance between the mixture density and the EM samples to find the best configuration.
...
A mixture model with fewer components may also provide the same quality of approximation, which would be even more computationally efficient.
...
However, we continue to emphasise that this example is demonstrative and the mixture model has only been proposed as an outline of an algorithm, and at this stage is highly ad-hoc.

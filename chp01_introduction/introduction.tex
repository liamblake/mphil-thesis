...
Many phenomena across geophysical, biological and socio-economic applications can be modelled using a continuous-time dynamical system, i.e.\ an ordinary differential equation \citep[e.g.]{BrauerCastillo-Chavez_2012_MathematicalModelsPopulation,TelEtAl_2005_ChemicalBiologicalActivity,Wiggins_2005_DynamicalSystemsApproach}.
...
The governing dynamics may be specified using existing phenomenological models, but in modern applications these are usually supplemented or driven by observed data \citep{LawEtAl_2015_DataAssimilationMathematical,ReichCotter_2015_ProbabilisticForecastingBayesian}.
...
The model provides a single prediction of the future state for any given initial condition.
...
\td{More generall,y uncertainty is prevalent across all field...}
For example, in weather and climate modelling resolution issues pose a significant challenge in producing accurate predictions.
...
One approach to systematically account for for these sources of uncertainty is by replacing the deterministic model with a \emph{stochastic} one, where the uncertainty is introduced as an inherit part of the model and the solutions are treated as random quantities.
...
\td{SP can improve forecasts: \citep{MitchellGottwald_2012_DataAssimilationSlow,HaEtAl_2015_ComparisonModelError}}
...
\td{Give some sort of explanation for what the next couple of sections do.}


The structure of this thesis beyond this introduction, and accordingly the contributions of each chapter, is given in \Cref{sec:intro_structure}
...
The solution is now itself a random quantity: rather than predicting a single value at a given time, the stochastic solution gives a probability distribution over the state space.
...
The most significant drawback of this simulation-based approach in practice is the computational cost.
...
Overcoming this significant limitation is an area of ongoing research in \emph{every} field in which such stochastic models are used.
...
\citet{Balasuriya_2020_StochasticSensitivityComputable} took one step towards addressing this deficiency, by providing a new set of computable tools for evaluating the impact of velocity field uncertainty on the evolution of trajectories.

However, these tools, and therefore the subsequent applications \citep{Balasuriya_2020_UncertaintyFinitetimeLyapunov,BadzaEtAl_2023_HowSensitiveAre,FangOuellette_2021_AssessingInformationContent,FangEtAl_2020_DisentanglingResolutionPrecision}, were limited to two-dimensions.
One of the primary aims of this thesis is to overcome this difficulty by further developing stochastic sensitivity as a theoretical tool into arbitrary dimensions.
We are able to do so by connecting stochastic sensitivity to another tool for analysing and approximating stochastic differential equations: linearisations.
...
\td{Where does our result fit in?}


The ease of computability of the solutions to a linearised SDE can itself be a drawback; the Gaussian distribution is limited in the feature which it can capture.
...
\begin{itemize}
	...
	\item In \Cref{ch:linear_numerics}, we numerically validate the results of \Cref{sec:theory} using stochastic simulations from a 2-dimensional model.
	...
	\item In \Cref{ch:appls}, we apply the tools developed in \Cref{ch:linear_theory,ch:gmm} to an example constructed from observed data: the position of a drifter on the surface of the North Atlantic ocean.
	      The aim here is to demonstrate the computability of the linearisation framework and stochastic sensitivity, and to illustrate that even a simple implementation of the mixture model algorithm in

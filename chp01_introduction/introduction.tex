...
Given an initial condition, these models can be olved, either analytically or numerically, to predict the behaviour at a future time.
...
One approach to systematically account for for uncertainty is by replacing the deterministic model with a \emph{stochastic} one, where the uncertainty is introduced as an inherit part of the model and the solutions are treated as random quantities.
...
This provide a new model to generate (now probabilistic) predictions and perform analysis or even to evaluating the reliability of the original deterministic model itself \citep{Balasuriya_2020_StochasticSensitivityComputable}.
...
However, to capture the stochasticity of the model, this requires taking many \emph{realisations} of the solution.
...
This further complicates usage of the model for prediction and analysis; such SDEs are intractable to solve analytically and generally more difficult to approximate accurately \citep{MoraEtAl_2017_StableNumericalScheme,SanchoEtAl_1982_AnalyticalNumericalStudies}.
...
Motivated by the need for efficient schemes actross applications, we specifically look to develop characterisations of and algorithms for approximating the probability distributions of solutions to such SDEs.
...
The simplicity of this Gaussian process can be a drawback, however; the unimodal Gaussian density is limited in the features which it can capture.
...
These trajectories solve an ODE involving he velocity field, which is inevitably subject to uncertainty from all the aforementioned sources.
...
This generalisation would have the additional advantage of extending stochastic sensitivity beyond the fluids context to any \(n\)-dimensional differential equation model, opening up a wealth of new applications.
...
\begin{itemize}
	...
	\item In \Cref{ch:linear_numerics}, we numerically validate the results of \Cref{ch:linear_theory} using stochastic simulations from 1- and 2-dimensional models.
	      ...
	      This, to the author's knowledge, is the first time that the validity of the linearisation approximation to a SDE has been systematically verified.
	...
	\item In \Cref{ch:gmm}, we outline an algorithm that employs the linearisation approximation to construct a Gaussian mixture model for the solution to a stochastic differential equation, which can capture non-Gaussianity while still maintaining computational efficiency.
	      ...
	      However, we demonstrate the potential of such an approach on the example of the following chapter.
